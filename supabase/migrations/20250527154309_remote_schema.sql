alter table "public"."judging_entries" add column "business_summary" text not null default ''::text;

alter table "public"."judging_entries" add column "general_comments_summary" text not null default ''::text;

alter table "public"."judging_entries" add column "innovation_summary" text not null default ''::text;

alter table "public"."judging_entries" add column "technical_summary" text not null default ''::text;

alter table "public"."judging_entries" add column "ux_summary" text not null default ''::text;

alter table "public"."judging_entries" alter column "judging_id" drop not null;
...
create or replace view "public"."top_people_view" as  SELECT u.id,
    u.email,
    u.full_name,
    u.avatar_url,
    u.created_at,
    u.updated_at,
    u.profile_header_url,
    u.terms_accepted,
    u.main_role,
    ...
    ((((
        CASE
            WHEN ((u.full_name IS NOT NULL) AND (u.avatar_url IS NOT NULL)) THEN 20
            ELSE 0
        END +
        CASE
            WHEN (((p.profile ->> 'description'::text) IS NOT NULL) AND (length(TRIM(BOTH FROM (p.profile ->> 'description'::text))) > 0)) THEN 20
            ELSE 0
        END) +
        CASE
            WHEN ((jsonb_typeof(((p.profile -> 'skills'::text) -> 'experience'::text)) = 'array'::text) AND (jsonb_array_length(((p.profile -> 'skills'::text) -> 'experience'::text)) > 0)) THEN 10
            ELSE 0
        END) +
        CASE
            WHEN ((jsonb_typeof(((p.profile -> 'skills'::text) -> 'technology'::text)) = 'array'::text) AND (jsonb_array_length(((p.profile -> 'skills'::text) -> 'technology'::text)) > 0)) THEN 10
            ELSE 0
        END) +
        CASE
            WHEN (jsonb_typeof((p.profile -> 'connected_accounts'::text)) = 'array'::text) THEN LEAST((jsonb_array_length((p.profile -> 'connected_accounts'::text)) * 5), 20)
            ELSE 0
        END) AS completion_percentage
   FROM ((users u
     JOIN LATERAL ( SELECT jsonb_agg(jsonb_build_object('is_primary', upr.is_primary, 'participant_roles', to_jsonb(pr.*))) AS roles
           FROM (user_participant_roles upr
             JOIN participant_roles pr ON ((pr.id = upr.role_id)))
          WHERE ((upr.participant_id = u.id) AND (upr.is_primary = true))) r ON (true))
     LEFT JOIN LATERAL ( SELECT to_jsonb(pp.*) AS profile
           FROM participant_profile pp
          WHERE (pp.participant_id = u.id)
         LIMIT 1) p ON (true));
...
create or replace view "public"."hackathon_participants_sorted" as  SELECT hp.id,
    hp.hackathon_id,
    hp.application_status,
    hp.created_at,
    hp.updated_at,
    hp.participant_id,
    hp.looking_for_teammates,
    row_to_json(tpv.*) AS users
   FROM (hackathon_participants hp
     JOIN top_people_view tpv ON ((tpv.id = hp.participant_id)));
