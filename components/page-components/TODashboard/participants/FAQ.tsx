...
const FAQSectionSkeletonloader = () => {
  return (
    <div className="mt-4">
...
interface FAQSectionProps extends ParticipantsAnalyticsPropsBase {}

const FAQ = (props: FAQSectionProps) => {
  const { hackathonId, technologyOwnerId } = props;

  const { data: faqs = [], isLoading } = useSWR<CountItem[]>(
    `/api/technology-owners/${technologyOwnerId}/hackathons/${hackathonId}/analytics/participants/faq`,
    fetcher
  );
  ...
  return (
    <div className="p-6 bg-secondary-bg rounded-2xl h-[360px] overflow-y-scroll border border-black-terciary">
      <header className="flex justify-between items-center">
        <p className="text-secondary-text">FAQ</p>
        <p className="text-secondary-text">Clicks</p>
      </header>

      <FadeTransitionLoader
        isLoading={isLoading}
        loader={<FAQSectionSkeletonloader />}
      >
        <div className="mt-4">
          <ul className="flex flex-col gap-3">
            {faqs?.map((item, index) => (
              <li
                ...
                key={index}
              >
                <span className="text-xs truncate max-w-[150px] text-white font-medium">
                  {item.label}
                </span>
